...
public class HttpClientXmlHandler : HttpClientMimeTypeHandler
{
	...
	private static readonly MediaTypeWithQualityHeaderValue AcceptMediaTypeTextXml = new(MediaTypeTextXml, quality: 1);

	private static readonly MediaTypeWithQualityHeaderValue AcceptMediaTypeApplicationXml = new(MediaTypeApplicationXml, quality: 1);

	private static bool CanHandle(string? mediaType) =>
		mediaType is not null &&
		(string.Equals(mediaType, MediaTypeApplicationXml, StringComparison.OrdinalIgnoreCase) ||
		 string.Equals(mediaType, MediaTypeTextXml, StringComparison.OrdinalIgnoreCase));
										...
										DataModelValue value)
	{
		...
		if (!headerValues.Contains(AcceptMediaTypeTextXml))
		{
			headerValues.Add(AcceptMediaTypeTextXml);
		}

		if (!headerValues.Contains(AcceptMediaTypeApplicationXml))
		{
			headerValues.Add(AcceptMediaTypeApplicationXml);
		}
	}
	...
	public override HttpContent? TryCreateHttpContent(HttpRequestMessage request,
													  ...
													  DataModelList parameters,
													  DataModelValue value) =>
		CanHandle(contentType?.MediaType) ? new XmlHttpContent(value) : null;
