...
[TestClass]
public class ParentEventDispatchIntegrationTest
{
	[TestMethod]
	[Timeout(10000)]
	public async Task InvokedScxmlSendToParentReachesParentStateMachine()
	{
		...
		var result = await scopeManager.Execute(new ScxmlStringStateMachine(parentScxml), SecurityContextType.NewStateMachine);

		Assert.AreEqual(expected: "parent-received-child-event", result.AsString());
	}
}
