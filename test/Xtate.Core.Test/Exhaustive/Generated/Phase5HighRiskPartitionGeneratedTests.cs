...
public sealed class Phase5HighRiskPartitionGeneratedTests
{
    ...
    */
    ...
    [
        new object[] { new Phase5PartitionCase("ROBUST-XML-001-CASE-101", "Bounded success or controlled diagnostic", "No crash, disclosure, unbounded allocation, or retained buffer.") },
        new object[] { new Phase5PartitionCase("PROP-CONFIG-001-CASE-101", "Exact reference trace and reproducible artifact", "No differential mismatch, session leak, or missing seed.") },
        new object[] { new Phase5PartitionCase("FAULT-STORE-001-CASE-101", "Primary fault plus exact cleanup", "No swallowed fault, later side effect, or leaked resource.") },
        new object[] { new Phase5PartitionCase("LEAK-SESSION-001-CASE-101", "Dead sentinels and baseline resource ledger", "No positive retention slope or unobserved finalizer error.") },
        new object[] { new Phase5PartitionCase("BUDGET-CANCEL-001-CASE-101", "Declared complexity and cancellation bounds", "No over-buffering, oversized diagnostic, or delayed cancellation.") },
        new object[] { new Phase5PartitionCase("CRASH-003-CASE-101", "Legal linearization and last-committed recovery", "No hybrid, duplicate/lost event, deadlock, or missing artifact.") }
    ];
