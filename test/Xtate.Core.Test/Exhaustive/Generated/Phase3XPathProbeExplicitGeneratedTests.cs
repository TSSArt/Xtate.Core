...
{
	private static readonly XPathProbeCase[] ExplicitCases =
	[
		new(
		...
		new(
			...
			Fixture:
			"Two isolated runtime-data-model sessions; first callback captures sentinel and completes, second callback checks no first-session ambient data, then bounded weak-reference collection runs.",
