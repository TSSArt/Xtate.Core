...
public sealed class Phase4HighRiskPartitionGeneratedTests
{
    ...
    */
    ...
    [
        new object[] { new Phase4PartitionCase("HOST-LIFE-004-CASE-101", "One removal, bounded waiters, isolated sibling", "No ghost event, duplicate disposal, or cross-session cancellation.") },
        new object[] { new Phase4PartitionCase("HOST-SCHED-002-CASE-101", "Only uncancelled group dispatches in order", "No cancelled dispatch, collateral cancellation, or unobserved task.") },
        new object[] { new Phase4PartitionCase("IO-HTTP-014-CASE-101", "Byte limit accepts N and rejects N+1 before dispatch", "No character sizing, partial dispatch, or retained stream.") },
        new object[] { new Phase4PartitionCase("IO-PIPE-004-CASE-101", "Bounded valid decode and controlled invalid-frame error", "No dangerous allocation, pool residue, or crash.") },
        new object[] { new Phase4PartitionCase("RES-SEC-001-CASE-101", "Denied acquisition makes zero provider calls", "No escalation, cache bypass, or retained buffer.") },
        new object[] { new Phase4PartitionCase("PERSIST-SUSP-006-CASE-101", "Recover last committed snapshot and replay once", "No hybrid, duplicate/lost send, or leaked lock.") }
    ];
