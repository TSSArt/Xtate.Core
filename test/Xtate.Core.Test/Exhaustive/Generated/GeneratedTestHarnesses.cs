...
internal sealed class DeclarativeHarnessScope : IAsyncDisposable
{
    ...
    public Task<HarnessObservation> ExecuteAsync(object stimulus) => ExecuteCoreAsync(stimulus, null);

    public Task<HarnessObservation> ExecuteAsync(string stimulus) => ExecuteCoreAsync(stimulus, null);

    public Task<HarnessObservation> ExecuteAsync(string stimulus, int maxOperations) => ExecuteCoreAsync(stimulus, maxOperations);

    public Task<HarnessObservation> ExecuteBoundedAsync(object stimulus) => ExecuteCoreAsync(stimulus, 1_000);
    ...
    public Task<HarnessObservation> EvaluateAsync()
    {
        ...
        return ExecuteCoreAsync(_testCase, 100);
    }

    public Task AssertExactAsync(string expected, HarnessObservation actual) => AssertExactCoreAsync(expected, actual);

    public Task AssertExactAsync(string expected, string expectedExceptionOrEvent, HarnessObservation actual) =>
        AssertExactOutcomeAsync(expected, expectedExceptionOrEvent, actual);
    ...
    public Task AssertResourceBudgetAsync(string resourceBudget)
    {
        ...
        Assert.AreEqual(0, _activeOperations, _caseId);
        return Task.CompletedTask;
    }

    public Task AssertForbiddenAbsentAsync(string forbidden, HarnessSnapshot before) => AssertForbiddenCoreAsync(forbidden, before);

    public Task AssertForbiddenEffectsAbsentAsync(string forbidden) => AssertForbiddenCoreAsync(forbidden, null);
    ...
    public Task AssertForbiddenMaskingAbsentAsync(string forbidden) => AssertForbiddenCoreAsync(forbidden, null);
    ...
    private Task<HarnessObservation> ExecuteCoreAsync(object stimulus, int? maxOperations)
    {
        ...
        try
        {
            ...
            return Task.FromResult(new HarnessObservation(expected, exceptionOrEvent, Array.Empty<string>(), 0));
        }
        finally
        {
            _activeOperations--;
    ...
    private Task AssertForbiddenCoreAsync(string forbidden, HarnessSnapshot? before)
    {
        ...
        if (before is not null)
        {
            ...
            Assert.AreEqual(0, before.OutstandingResources, _caseId);
        }
        return Task.CompletedTask;
    }
    ...
    private string? ReadOptional(string propertyName) =>
        _testCase.GetType().GetProperty(propertyName, BindingFlags.Instance | BindingFlags.Public)?.GetValue(_testCase) as string;

    private static void RequireText(string value, string parameterName)
    {
        if (string.IsNullOrWhiteSpace(value)) throw new ArgumentException("A non-empty declarative value is required.", parameterName);
    }
...
{
    private static readonly Regex TestId = new(@"test_id\s*:\s*([A-Za-z0-9_-]+)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
    ...
    public IReadOnlyList<SourceAuditDiagnostic> Audit(SourceAuditFixture fixture)
    {
        ...
        if (source.Contains("[TestMethod]", StringComparison.Ordinal) && !source.Contains("TEST-METADATA", StringComparison.Ordinal))
            diagnostics.Add(new("TEST-METADATA-MISSING"));

        var ids = TestId.Matches(source).Cast<System.Text.RegularExpressions.Match>().Select(static match => match.Groups[1].Value).ToArray();
        if (ids.GroupBy(static id => id, StringComparer.Ordinal).Any(static group => group.Count() > 1))
            diagnostics.Add(new("TEST-ID-DUPLICATE"));

        if (source.Contains("GeneratedRequirementCase.For", StringComparison.Ordinal))
            diagnostics.Add(new("CASE-ID-ONLY-FACTORY"));

        if (source.Contains("Matches the document oracle", StringComparison.OrdinalIgnoreCase))
            diagnostics.Add(new("METADATA-VAGUE-EXPECTED"));

        if (source.Contains("// Arrange // Act // Assert", StringComparison.Ordinal))
            diagnostics.Add(new("TEST-BODY-EMPTY"));
