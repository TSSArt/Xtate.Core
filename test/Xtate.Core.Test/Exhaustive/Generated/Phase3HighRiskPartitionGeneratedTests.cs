...
public sealed class Phase3HighRiskPartitionGeneratedTests
{
    ...
    */
    ...
    [
        new object[] { new Phase3PartitionCase("DM-VALUE-001-CASE-101", "Exact kind-preserving round trip", "No null/undefined aliasing or silent stringification.") },
        new object[] { new Phase3PartitionCase("DM-LIST-001-CASE-101", "Ordered deep copy with metadata", "No shared child identity or reordered entries.") },
        new object[] { new Phase3PartitionCase("DM-RUNTIME-001-CASE-101", "Handler and ambient isolation", "No cross-session context or null-model evaluation.") },
        new object[] { new Phase3PartitionCase("XPATH-TREE-001-CASE-101", "Lexical namespace capture", "No late namespace lookup or context reuse.") },
        new object[] { new Phase3PartitionCase("XPATH-EXPR-014-CASE-101", "XPath effective boolean values", "No lexical Boolean parsing or NaN truthiness.") },
        new object[] { new Phase3PartitionCase("XPATH-SYS-004-CASE-101", "System variable immutability and In conversion", "No partial reserved-variable mutation.") },
        new object[] { new Phase3PartitionCase("XPATH-ASSIGN-010-CASE-101", "Eight-action rollback", "No partial target mutation or repeated value evaluation.") },
        new object[] { new Phase3PartitionCase("XPATH-ASSIGN-010-CASE-102", "Delete rollback preserves both targets after the second target rejects mutation", "No first-target deletion, partial tree, or retained transaction.") },
        new object[] { new Phase3PartitionCase("XPATH-FOREACH-001-CASE-101", "One-based shallow-copy iteration", "No zero-based index or scope leak.") },
        new object[] { new Phase3PartitionCase("XPATH-CONTENT-004-CASE-101", "Content fidelity and cached failure", "No truncation, repeated acquisition, or retained stream.") },
        new object[] { new Phase3PartitionCase("DM-PROP-003-CASE-101", "Mutation differential with reproducible seed", "No mismatch, partial mutation, or missing reproducer.") }
    ];
