...
public sealed class ScxmlParserRequirementsTests
{
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"idle\" /></scxml>", "urn:exhaustive:parse-001");

		Assert.IsTrue(result.Accepted, Describe(result));
		Assert.AreEqual(1, result.Model!.States.Length);
		Assert.AreEqual("idle", result.Model.States[0].Id!.Value);
	}
	...
	[TestMethod]
		...
		*/
		...
		public async Task SCXML_PARSE_001_Accepts_an_xml_declaration_before_the_scxml_root()
		{
				...
				Assert.IsTrue(result.Accepted, Describe(result));
				Assert.AreEqual("ready", result.Model!.States[0].Id!.Value);
		}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync(document, "urn:exhaustive:parse-001-invalid");
		Assert.IsFalse(result.Accepted, Describe(result));
		Assert.IsNull(result.Model, "A rejected root name or namespace must not expose a usable SCXML model.");
	}
	...
	*/
	...
	public async Task SCXML_PARSE_003_Preserves_root_initial_name_and_binding_attributes()
	{
		...
		Assert.IsTrue(result.Accepted, Describe(result));
		Assert.AreEqual("unicode-Δ", result.Model!.Name);
		Assert.AreEqual(Xtate.StateMachine.BindingType.Late, result.Model.Binding);
		Assert.IsNotNull(result.Model.Initial);
				Assert.AreEqual("idle", result.Model.Initial.Transition!.Target.Array[0].Value);
		}
		...
		*/
		...
		{
				var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" xmlns:xml=\"http://www.w3.org/XML/1998/namespace\" xml:base=\"https://example.invalid/base/\" version=\"1.0\"><state id=\"ready\" /></scxml>");
	...
	*/
	...
	public async Task SCXML_PARSE_004_Preserves_legal_root_state_and_final_child_order_while_ignoring_comments_and_processing_instructions()
	{
		...
		Assert.IsTrue(result.Accepted, Describe(result));
		Assert.AreEqual(2, result.Model!.States.Length);
		Assert.AreEqual("first", result.Model.States[0].Id!.Value);
		Assert.AreEqual("last", result.Model.States[1].Id!.Value);
	}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync(document, "urn:exhaustive:malformed");
	...
	*/
	...
	public async Task SCXML_PARSE_024_Prohibits_document_type_declarations_before_any_external_entity_is_resolved()
	{
		...
		var result = await ScxmlParserHarness.ParseAsync(document, "urn:exhaustive:xxe");
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync(document, "urn:exhaustive:malformed-extra");
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync("<scxml xmlns=\"http://www.w3.org/2005/07/scxml\" version=\"1.0\"><state id=\"s\"></scxml>", "urn:exhaustive:mismatched");
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"state\"><onentry><send event=\"event\" delay=\"{lexicalDelay}\" /></onentry></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var state = (Xtate.StateMachine.IState)result.Model!.States[0];
		var send = (Xtate.StateMachine.ISend)state.OnEntry[0].Action[0];
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"state\"><onentry><send event=\"event\" delay=\"1.5s\" /></onentry></state></scxml>");
		Assert.IsFalse(result.Accepted, Describe(result));
	}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"state\"><onentry><send event=\"event\" delay=\"2147484s\" /></onentry></state></scxml>");
		Assert.IsFalse(result.Accepted, Describe(result));
	}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"state\"><onentry><send event=\"event\" delay=\" 1s\" /></onentry></state></scxml>");
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"ready\"><onentry><send event=\"tick\" delayexpr=\"'1s'\" /></onentry></state></scxml>");
	...
	*/
	...
	public async Task SCXML_PARSE_023_Accepts_a_non_seekable_one_byte_async_stream_without_partial_model()
	{
		...
		var result = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:one-byte");

		Assert.IsTrue(result.Accepted, Describe(result));
		Assert.IsTrue(stream.ReadCalls > 1, "The fixture must force incremental reads.");
		Assert.IsTrue(stream.CanRead, "The parser does not own the caller-provided stream.");
	}
	...
	*/
	...
	public async Task SCXML_PARSE_021_And_PARSE_023_Read_failure_after_partial_input_returns_no_model()
	{
		...
		var result = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:read-failure");

		Assert.IsNull(result.Model, Describe(result));
		Assert.IsFalse(result.Accepted, Describe(result));
		Assert.IsTrue(stream.CanRead, "A parser failure must not dispose the caller-owned stream.");
	}
	...
	*/
	...
	public async Task SCXML_PARSE_022_Accepts_utf16_little_endian_with_bom_and_preserves_unicode_identifiers()
	{
		...
		Buffer.BlockCopy(encodedXml, 0, bytes, preamble.Length, encodedXml.Length);

		using var stream = new MemoryStream(bytes, writable: false);
		var result = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:utf16");

		Assert.IsTrue(result.Accepted, Describe(result));
		Assert.AreEqual("Δstate", result.Model!.States[0].Id!.Value);
	}
	...
	*/
	...
	public async Task SCXML_PARSE_022_Accepts_utf16_big_endian_with_bom_and_preserves_unicode_identifiers()
	{
		...
		using var stream = new MemoryStream(bytes, writable: false);
		var result = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:utf16be");

		Assert.IsTrue(result.Accepted, Describe(result));
		Assert.AreEqual("漢字", result.Model!.States[0].Id!.Value);
	}
	...
	*/
	...
	public async Task SCXML_PARSE_022_Accepts_utf8_without_a_bom_and_preserves_unicode_identifiers()
	{
		...
		var result = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:utf8-no-bom");

		Assert.IsTrue(result.Accepted, Describe(result));
		Assert.AreEqual("café", result.Model!.States[0].Id!.Value);
	}
	...
	*/
	...
	public async Task SCXML_PARSE_021_Rejects_an_incomplete_utf8_sequence_without_returning_a_model()
	{
		...
		var suffix = Encoding.UTF8.GetBytes(" /></scxml>");
		...
		Buffer.BlockCopy(suffix, 0, bytes, prefix.Length + 2, suffix.Length);

		using var stream = new MemoryStream(bytes, writable: false);
		var result = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:invalid-utf8");
	...
	*/
	...
	public async Task SCXML_PARSE_022_Rejects_an_xml_encoding_declaration_that_mismatches_the_bytes()
	{
		...
		using var stream = new MemoryStream(Encoding.UTF8.GetBytes(xml), writable: false);
		var result = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:encoding-mismatch");
	...
	*/
	...
	public async Task SCXML_PARSE_023_A_second_parse_attempt_on_a_consumed_stream_does_not_reuse_the_first_model()
	{
		...
		var first = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:repeat-first");
		var second = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:repeat-second");
	...
	*/
	...
	public async Task SCXML_PARSE_023_Does_not_close_a_caller_owned_stream()
	{
		...
		var result = await ScxmlParserHarness.ParseStreamAsync(stream, "urn:exhaustive:caller-owned");
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"source\"><transition event=\"alpha beta\" target=\"first second\" type=\"internal\" /></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));
		...
		Assert.AreEqual(Xtate.StateMachine.TransitionType.Internal, transition.Type);
	}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"state\"><onentry><raise event=\"raised.event\" /><log label=\"checkpoint\" expr=\"value\" /></onentry></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var state = (Xtate.StateMachine.IState)result.Model!.States[0];
		var actions = state.OnEntry[0].Action;
		Assert.IsInstanceOfType<Xtate.StateMachine.IRaise>(actions[0]);
		Assert.IsInstanceOfType<Xtate.StateMachine.ILog>(actions[1]);
		Assert.AreEqual("checkpoint", ((Xtate.StateMachine.ILog)actions[1]).Label);
	}
	...
	*/
	...
	public async Task SCXML_PARSE_006_Preserves_nested_compound_state_structure_and_ids()
	{
		...
		var parent = (Xtate.StateMachine.IState)result.Model!.States[0];
		Assert.AreEqual("parent", parent.Id!.Value);
		Assert.AreEqual(1, parent.States.Length);
		Assert.AreEqual("child", parent.States[0].Id!.Value);
	}
	...
	*/
	...
	public async Task SCXML_PARSE_006_Preserves_a_compound_state_initial_attribute()
	{
		...
		Assert.AreEqual("child", parent.Initial!.Transition!.Target.Array[0].Value);
	}
	...
	*/
	...
	public async Task SCXML_PARSE_008_Parses_root_final_as_a_final_state_with_its_identifier()
	{
		...
		Assert.IsInstanceOfType<Xtate.StateMachine.IFinal>(result.Model!.States[0]);
		Assert.AreEqual("complete", result.Model.States[0].Id!.Value);
	}
	...
	*/
	...
	public async Task SCXML_PARSE_007_Parses_parallel_regions_in_document_order()
	{
		...
		Assert.IsInstanceOfType<Xtate.StateMachine.IParallel>(result.Model!.States[0]);
		var parallel = (Xtate.StateMachine.IParallel)result.Model.States[0];
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"parent\"><initial><transition target=\"child\" /></initial><state id=\"child\" /></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"parent\"><history id=\"resume\" type=\"deep\"><transition target=\"child\" /></history><state id=\"child\" /></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var parent = (Xtate.StateMachine.IState)result.Model!.States[0];
		Assert.AreEqual(1, parent.HistoryStates.Length);
		var history = parent.HistoryStates[0];
		Assert.AreEqual("resume", history.Id!.Value);
		Assert.AreEqual(Xtate.StateMachine.HistoryType.Deep, history.Type);
		Assert.AreEqual("child", history.Transition!.Target.Array[0].Value);
	}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"active\"><onentry><if cond=\"first\"><raise event=\"one\" /><elseif cond=\"second\" /><log label=\"two\" /><else /><raise event=\"three\" /></if></onentry></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var state = (Xtate.StateMachine.IState)result.Model!.States[0];
		var conditional = (Xtate.StateMachine.IIf)state.OnEntry[0].Action[0];
		Assert.IsNotNull(conditional.Condition);
		Assert.AreEqual(5, conditional.Action.Length);
		Assert.IsInstanceOfType<Xtate.StateMachine.IRaise>(conditional.Action[0]);
		Assert.IsInstanceOfType<Xtate.StateMachine.IElseIf>(conditional.Action[1]);
		Assert.IsInstanceOfType<Xtate.StateMachine.ILog>(conditional.Action[2]);
		Assert.IsInstanceOfType<Xtate.StateMachine.IElse>(conditional.Action[3]);
		Assert.IsInstanceOfType<Xtate.StateMachine.IRaise>(conditional.Action[4]);
		Assert.AreEqual("one", ((Xtate.StateMachine.IRaise)conditional.Action[0]).OutgoingEvent!.Name.ToString());
		Assert.AreEqual("two", ((Xtate.StateMachine.ILog)conditional.Action[2]).Label);
		Assert.AreEqual("three", ((Xtate.StateMachine.IRaise)conditional.Action[4]).OutgoingEvent!.Name.ToString());
	}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"active\"><onentry><foreach array=\"items\" item=\"item\" index=\"position\"><log label=\"each\" /></foreach></onentry></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var state = (Xtate.StateMachine.IState)result.Model!.States[0];
		var loop = (Xtate.StateMachine.IForEach)state.OnEntry[0].Action[0];
		Assert.IsNotNull(loop.Array);
		Assert.IsNotNull(loop.Item);
		Assert.IsNotNull(loop.Index);
		Assert.AreEqual(1, loop.Action.Length);
		Assert.IsInstanceOfType<Xtate.StateMachine.ILog>(loop.Action[0]);
		Assert.AreEqual("each", ((Xtate.StateMachine.ILog)loop.Action[0]).Label);
	}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"active\"><onentry><send event=\"notify\" target=\"#_parent\" type=\"http://www.w3.org/TR/scxml/#SCXMLEventProcessor\" id=\"outbound\" delay=\"2s\" namelist=\"first second\"><param name=\"alpha\" expr=\"value\" /><param name=\"beta\" location=\"source\" /><content>payload</content></send></onentry></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var state = (Xtate.StateMachine.IState)result.Model!.States[0];
		var send = (Xtate.StateMachine.ISend)state.OnEntry[0].Action[0];
		Assert.AreEqual("notify", send.EventName);
		Assert.AreEqual("#_parent", send.Target!.OriginalString);
		Assert.AreEqual("http://www.w3.org/TR/scxml/#SCXMLEventProcessor", send.Type!.OriginalString);
		Assert.AreEqual("outbound", send.Id);
		Assert.AreEqual(2000, send.DelayMs);
		Assert.AreEqual(2, send.NameList.Length);
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"active\"><invoke type=\"http://www.w3.org/TR/scxml/\" src=\"https://example.test/child.scxml\" id=\"child\" namelist=\"first second\" autoforward=\"true\"><param name=\"input\" expr=\"value\" /><finalize><log label=\"finish\" /></finalize><content>inline child</content></invoke></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var state = (Xtate.StateMachine.IState)result.Model!.States[0];
		Assert.AreEqual(1, state.Invoke.Length);
		var invoke = state.Invoke[0];
		Assert.AreEqual("http://www.w3.org/TR/scxml/", invoke.Type!.OriginalString);
		Assert.AreEqual("https://example.test/child.scxml", invoke.Source!.OriginalString);
		Assert.AreEqual("child", invoke.Id);
		Assert.IsTrue(invoke.AutoForward);
		Assert.AreEqual(2, invoke.NameList.Length);
		Assert.AreEqual(1, invoke.Parameters.Length);
		Assert.AreEqual("input", invoke.Parameters[0].Name);
		Assert.IsNotNull(invoke.Parameters[0].Expression);
		Assert.IsNotNull(invoke.Finalize);
		Assert.AreEqual(1, invoke.Finalize!.Action.Length);
		Assert.IsInstanceOfType<Xtate.StateMachine.ILog>(invoke.Finalize.Action[0]);
		Assert.AreEqual("finish", ((Xtate.StateMachine.ILog)invoke.Finalize.Action[0]).Label);
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><datamodel><data id=\"calculated\" expr=\"value\" /><data id=\"inline\">text payload</data></datamodel><final id=\"complete\"><donedata><param name=\"result\" expr=\"calculated\" /><content>done payload</content></donedata></final></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var dataModel = result.Model!.DataModel;
		Assert.IsNotNull(dataModel);
		Assert.AreEqual(2, dataModel!.Data.Length);
		Assert.AreEqual("calculated", dataModel.Data[0].Id);
		Assert.IsNotNull(dataModel.Data[0].Expression);
		Assert.AreEqual("inline", dataModel.Data[1].Id);
		Assert.IsNotNull(dataModel.Data[1].InlineContent);

		var final = (Xtate.StateMachine.IFinal)result.Model.States[0];
		Assert.IsNotNull(final.DoneData);
		Assert.AreEqual(1, final.DoneData!.Parameters.Length);
		Assert.AreEqual("result", final.DoneData.Parameters[0].Name);
	...
	*/
	...
	public async Task SCXML_PARSE_018_Preserves_foreign_executable_action_namespace_name_and_outer_xml()
	{
		...
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" xmlns:ext=\"{ExtensionNamespace}\" version=\"1.0\"><state id=\"active\"><onentry><ext:record level=\"audit\"><ext:detail>value</ext:detail></ext:record></onentry></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var state = (Xtate.StateMachine.IState)result.Model!.States[0];
		var custom = (Xtate.StateMachine.ICustomAction)state.OnEntry[0].Action[0];
		Assert.AreEqual(ExtensionNamespace, custom.XmlNamespace);
		Assert.AreEqual("record", custom.XmlName);
		Assert.IsNotNull(custom.Xml);
		StringAssert.Contains(custom.Xml, "level=\"audit\"");
		StringAssert.Contains(custom.Xml, "detail");
		StringAssert.Contains(custom.Xml, "value");
	}
	...
	*/
	...
	public async Task SCXML_PARSE_019_Tokenizes_initial_target_event_and_namelist_on_xml_whitespace()
	{
		...
		var first = (Xtate.StateMachine.IState)result.Model.States[0];
		...
		var second = (Xtate.StateMachine.IState)result.Model.States[1];
		var send = (Xtate.StateMachine.ISend)second.OnEntry[0].Action[0];
		Assert.AreEqual(3, send.NameList.Length);
	}
	...
	*/
	...
	{
		var result = await ScxmlParserHarness.ParseAsync($"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\"><state id=\"active\"><onentry /><onexit><log label=\"leaving\" /><raise event=\"exited\" /></onexit></state></scxml>");
		Assert.IsTrue(result.Accepted, Describe(result));

		var state = (Xtate.StateMachine.IState)result.Model!.States[0];
		Assert.AreEqual(1, state.OnEntry.Length);
		Assert.IsTrue(state.OnEntry[0].Action.IsDefaultOrEmpty);
		Assert.AreEqual(1, state.OnExit.Length);
		Assert.AreEqual(2, state.OnExit[0].Action.Length);
		Assert.IsInstanceOfType<Xtate.StateMachine.ILog>(state.OnExit[0].Action[0]);
		Assert.IsInstanceOfType<Xtate.StateMachine.IRaise>(state.OnExit[0].Action[1]);
		Assert.AreEqual("leaving", ((Xtate.StateMachine.ILog)state.OnExit[0].Action[0]).Label);
		Assert.AreEqual("exited", ((Xtate.StateMachine.IRaise)state.OnExit[0].Action[1]).OutgoingEvent!.Name.ToString());
	}
	...
	*/
	...
	{
		const string source = $"<scxml xmlns=\"{ScxmlNamespace}\" version=\"1.0\" name=\"parity\" initial=\"idle\"><state id=\"idle\"><transition event=\"go\" target=\"done\" /></state><final id=\"done\" /></scxml>";
		...
		CollectionAssert.AreEqual(originalState.Transitions[0].EventDescriptors.Array.Select(static item => item.Value).ToArray(), roundTripState.Transitions[0].EventDescriptors.Array.Select(static item => item.Value).ToArray());
	}
	...
	private static string Describe(ScxmlParserHarness.ParseResult result) =>
		$"Accepted={result.Accepted}; Diagnostics=[{string.Join(" | ", result.Diagnostics)}]; Exception={result.Exception}";
}
